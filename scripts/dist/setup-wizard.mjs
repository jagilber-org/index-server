...
function resolveServerLaunch(config) {
  ...
  if (fs.existsSync(localEntry)) {
    return { command: 'node', args: [entryRelative], cwd: config.root, source: 'local' };
  }
  if (fs.existsSync(packagedEntry)) {
    return { command: 'node', args: [fwd(packagedEntry)], cwd: config.root, source: 'packaged' };
  }
  ...
  return { command: 'npx', args: ['-y', `${pkgName}${pkgVersion}`], cwd: config.root, source: 'npx' };
}
...
/** Run an npm command. Uses npm CLI script if found, otherwise executes `npm` on PATH without a shell. */
...
Non-interactive mode:
  ...
  console.log(`     ${path.join(config.root, '.vscode', 'mcp.json.generated')}\n`);
  step++;
