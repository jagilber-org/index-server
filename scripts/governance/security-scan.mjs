...
function loadPiiFileAllowlist() {
  const filePath = join(process.cwd(), '.pii-file-allowlist');
...
const EXCLUDE = ['node_modules', 'tmp', 'test-results', 'coverage', 'dist', '.git', 'data', 'feedback', 'metrics', 'backups', '.private', '.squad', '.squad-templates', '.codeql'];
...
const PII_FILE_ALLOWLIST = [
  ...
  'mermaid.min.js',
  'package-lock.json',
  // Generated by New-CleanRoomCopy.ps1; contains SHA-256 contentHash + commit SHA which trip
  // hex-thumbprint heuristics. Verified by Publish-ToMirror.ps1 hash gate; no real secrets.
  '.publish-manifest.json',
  'security-scan.mjs',
  'test_results.txt',
  // The allowlist file itself catalogs the very patterns it allows -- skip self-match.
  '.pii-allowlist',
  // Pre-commit hooks define PII detection regex patterns (not actual secrets).
  'pre-commit.mjs',
  'pre-commit.ps1',
];
...
const piiFileAllowlist = loadPiiFileAllowlist();
...
for (const f of files) {
  ...
  if (PII_FILE_ALLOWLIST.includes(basename) || piiFileAllowlist.includes(relPath) || PII_PATH_ALLOWLIST.some(pattern => pattern.test(relPath)) || testIsBinaryFile(f)) continue;
  try {
