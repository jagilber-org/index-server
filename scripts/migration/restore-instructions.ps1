...
.PARAMETER Destination
  Root deployment directory containing backups/ and instructions/.
...
.EXAMPLE
  ...
  pwsh scripts/restore-instructions.ps1 -Destination <production-install-root> -BackupName backup-instructions-20260330-215147.zip

.EXAMPLE
  # Restore from external backup zip (e.g., rename script backup):
  pwsh scripts/restore-instructions.ps1 -Destination <production-install-root> -BackupPath <absolute-backup-zip-path> -Force

.EXAMPLE
  # Preview restore without writing:
  pwsh scripts/restore-instructions.ps1 -Destination <production-install-root> -WhatIf
#>
