...
$prodRoot = if ($env:DEPLOY_PROD_PATH) { $env:DEPLOY_PROD_PATH } else { Join-Path $localMachineRoot 'index-server-prod' }
$testFile = Join-Path $prodRoot 'instructions\test-contenttype-migration-temp.json'
...
$migrationCode = Get-Content (Join-Path $prodRoot 'dist\versioning\schemaVersion.js') -Raw
