...
.DESCRIPTION
    ...
    remote Git repository via force-push (DirectPublish), restricted review repo
...
.PARAMETER CreateReviewRepo
    Create a restricted review repo on GitHub for inspection (requires gh CLI).

.PARAMETER ReviewOrg
    GitHub organization for restricted review repos.
...
if ($DryRun) {
    ...
    elseif ($CreatePR) {
        ...
        Write-Host "[DRY RUN] Would create restricted review repo under $ReviewOrg"
    }
    return
}
...
try {
    ...
    elseif ($CreatePR) {
        ...
        if ($LASTEXITCODE -eq 0 -or $prAlreadyExisted) {
            ...
            if ($prAlreadyExisted) {
                Write-Host ' PULL REQUEST UPDATED (existing PR for branch reused)' -ForegroundColor Cyan
            } else {
        ...
        if (-not $PSCmdlet.ShouldProcess("$ReviewOrg/$reviewRepoName", 'Create restricted review repo')) {
            Write-Host 'Aborted.'
            return
        }
