...
// Check for public remote and validate against .publish-config.json
if (!verifyOnly) {
  try {
    ...
    // Validate remote URL against .publish-config.json sanctioned remotes
    const publishConfigPath = path.join(repoRoot, '.publish-config.json');
    if (fs.existsSync(publishConfigPath)) {
      const publishConfig = JSON.parse(fs.readFileSync(publishConfigPath, 'utf8'));
      const publicUrl = runGit(['remote', 'get-url', 'public']).trim().replace(/\.git$/, '');
      const sanctioned = (publishConfig.sanctionedRemotes || []).map(u => u.replace(/\.git$/, ''));

      if (sanctioned.length > 0 && !sanctioned.includes(publicUrl)) {
        console.error(`ERROR: Public remote URL "${publicUrl}" is not in .publish-config.json sanctionedRemotes.`);
        console.error(`Allowed: ${sanctioned.join(', ')}`);
        process.exit(1);
      }

      if (!dryRun && publishConfig.allowDirectPublish === false) {
        console.error('ERROR: Direct publish is disabled in .publish-config.json (allowDirectPublish: false).');
        console.error('Use the two-step workflow: New-CleanRoomCopy.ps1 then Publish-ToMirror.ps1');
        process.exit(1);
      }
    } else {
      console.error('ERROR: .publish-config.json not found. Cannot validate publish target.');
      console.error('Create .publish-config.json with sanctionedRemotes to authorize publish targets.');
      process.exit(1);
    }
  } catch (e) {
    if (e.status !== undefined) throw e; // re-throw process.exit
...
Files: ${fileCount}`;
  ...
  // See upstream template issue #47.
