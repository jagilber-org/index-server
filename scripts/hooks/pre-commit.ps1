...
function Get-TargetFiles {
  ...
  if ($CandidateFiles -and $CandidateFiles.Count -gt 0) {
    return $CandidateFiles | Where-Object {
      $_ -and (Test-Path $_) -and ($_ -notmatch '^scripts/hooks/pre-commit\.(ps1|mjs)$')
    }
  }
...
foreach($item in (Get-ChildItem env:)) {
  ...
  if ($processEnvValueAllowlist -contains $value.ToLower()) { continue }
...
# .env.example. Tokens come from KEY=VALUE pairs in .env. Path-shaped values
# are always PII.
...
function Test-LooksLikePathValue {
  param([string]$Value)
...
if (Test-Path $dotenvPath) {
  ...
  foreach ($dotenvLine in (Get-Content $dotenvPath -ErrorAction SilentlyContinue)) {
    if ($dotenvLine -match '^\s*([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.*)\s*$') {
      ...
      $isDotenvPathValue = Test-LooksLikePathValue -Value $dotenvVal
      # Full-value token: catches every path plus non-path secrets >=16 chars.
      if ($isDotenvPathValue -or $dotenvVal.Length -ge $minDotenvTokenLen) {
      ...
      foreach ($pat in $userPathPatterns) {
        if ($dotenvVal -match $pat) {
          $userTok = $matches[1]
          if ($userTok -and $userTok.Length -ge 3) {
...
foreach($file in $targets) {
  ...
  if ($normalized -match '(^|/)\.env-leak-allowlist$') {
...
if ($errors -gt 0) {
  ...
  Write-Host '║                                                            ║' -ForegroundColor Red
  Write-Host '║  Use .pii-allowlist or # pii-allowlist for PII false positives.║' -ForegroundColor Red
