 ...
 *   4. Path-shaped values loaded from local .env files
 *
...
const PATH_EXCLUSIONS = new Set([
  ...
  'scripts/build/publish-direct-to-remote.cjs',
]);
...
const PROCESS_ENV_VALUE_ALLOWLIST = new Set([
  ...VALUE_ALLOWLIST,
  // Generic runtime default; .env path values are still checked below.
  './exports',
]);
...
for (const [key, value] of Object.entries(process.env)) {
  ...
  if (PROCESS_ENV_VALUE_ALLOWLIST.has(value.toLowerCase())) {
    continue;
  }
...
function testLooksLikePath(value) {
  return DOTENV_PATH_PATTERNS.some(pattern => pattern.test(value));
}
...
if (existsSync(dotenvPath)) {
  ...
  for (const dotenvLine of dotenvText.split(/\r?\n/)) {
    ...
    const isDotenvPathValue = testLooksLikePath(dotenvVal);
    if (isDotenvPathValue || dotenvVal.length >= minDotenvTokenLen) {
...
for (const file of targets) {
  ...
  if ((/(^|\/)\.env-leak-allowlist$/).test(normalized)) {
    fail(`Forbidden env leak allowlist file committed: ${file}`);
    continue;
  }
...
if (process.env.SKIP_PRE_COMMIT_FRAMEWORK !== '1') {
  ...
  let preCommitCommand = 'pre-commit';
  try {
    const located = execSync(process.platform === 'win32' ? 'where pre-commit' : 'command -v pre-commit', {
      encoding: 'utf8',
    }).split(/\r?\n/).map(line => line.trim()).filter(Boolean);
    if (located[0]) preCommitCommand = located[0];
...
if (errors > 0) {
  ...
  console.error('║                                                            ║');
  console.error('║  Use .pii-allowlist or # pii-allowlist for PII false positives.║');
