...
async function main() {
  ...
  const env = { ...process.env, INDEX_SERVER_DASHBOARD: '1', DASHBOARD_PORT: opts.port, INDEX_SERVER_DIR: process.env.INDEX_SERVER_DIR || process.env.INDEX_SERVER_DIR || `${process.cwd()}/devinstructions` };
  ...
  if (process.platform === 'win32') {
    // Spawn npx directly with args array — avoids shell metacharacter injection via cmd.exe
    pw = spawn('npx', pwArgs, { env: pwEnv, stdio: 'inherit', shell: true });
  } else {
    const npxCmd = 'npx';
