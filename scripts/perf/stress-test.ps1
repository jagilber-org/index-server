...
$validation = Join-Path $scriptDir 'crud-response-validation.ps1'
...
if ($healthErrors.Count -gt 0) {
    Write-Error "Server health validation failed at ${BaseUrl}: $($healthErrors -join '; ')"
    exit 1
}
...
if ($EmbeddingsCompute) {
    ...
    $sw.Stop()
    $embErrors = @(Test-EmbeddingComputeResponse -Response $emb)
    if ($embErrors.Count -eq 0) {
        ...
        Write-Host ("  WARN: {0}" -f ($embErrors -join '; ')) -ForegroundColor Yellow
    }
}
...
if ($EmbeddingsProjection) {
    ...
    $sw.Stop()
    $projErrors = @(Test-EmbeddingProjectionResponse -Response $proj)
    if ($projErrors.Count -eq 0) {
        ...
        Write-Host ("  WARN: {0}" -f ($projErrors -join '; ')) -ForegroundColor Yellow
    }
}
...
function Run-Cycle {
    ...
    Add-Errors ([ref]$errors) 'ADD' @(Test-AddResponse -Response $addResult -Id $id -ExpectedMutation created)
    ...
    Add-Errors ([ref]$errors) 'GET' @(Test-GetResponse -Response $getResult -Id $id -ExpectedTitle $title -ExpectedBodyContains "iteration $i")
    ...
    Add-Errors ([ref]$errors) 'SEARCH' @(Test-SearchResponse -Response $searchResult -Id $id)
    ...
    if ($SemanticSearch) {
        ...
        Add-Errors ([ref]$errors) 'SEMANTIC' @(Test-SearchResponse -Response $semResult -MinMatches 1)
    }
    ...
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    $updateResult = Invoke-Client @{ Action = 'add'; Id = $id; Title = $updatedTitle; Body = $updatedBody; Overwrite = $true }
    $sw.Stop()
    $timings['update'] = $sw.ElapsedMilliseconds
    Add-Errors ([ref]$errors) 'UPDATE' @(Test-AddResponse -Response $updateResult -Id $id -ExpectedMutation overwritten)

    # VERIFY UPDATE
    $sw = [System.Diagnostics.Stopwatch]::StartNew()
    $verifyUpdateResult = Invoke-Client @{ Action = 'get'; Id = $id }
    $sw.Stop()
    $timings['verify_update'] = $sw.ElapsedMilliseconds
    Add-Errors ([ref]$errors) 'VERIFY_UPDATE' @(Test-GetResponse -Response $verifyUpdateResult -Id $id -ExpectedTitle $updatedTitle -ExpectedBodyContains 'UPDATED')
    ...
    if (-not $NoDelete) {
        ...
        Add-Errors ([ref]$errors) 'REMOVE' @(Test-RemoveResponse -Response $removeResult -Id $id)
        ...
        Add-Errors ([ref]$errors) 'VERIFY_DELETED' @(Test-DeletedResponse -Response $verifyResult -Id $id)
    }
...
if ($Parallel -le 1) {
    ...
    $results = 1..$Iterations | ForEach-Object -ThrottleLimit $Parallel -Parallel {
        ...
        $invokeClient = {
            ...
            $merged = $cArgs.Clone()
            foreach ($k in $Params.Keys) { $merged[$k] = $Params[$k] }
            ConvertFrom-ClientOutput (& $clientPath @merged 2>&1)
        }
        $addErrors = {
            param([string]$Operation, [string[]]$ValidationErrors)
            foreach ($e in @($ValidationErrors)) { [void]$errors.Add("${Operation}: $e") }
        }
        ...
        $sw.Stop()
        $timings['add'] = $sw.ElapsedMilliseconds
        & $addErrors 'ADD' @(Test-AddResponse -Response $addResult -Id $id -ExpectedMutation created)
        ...
        $sw.Stop()
        $timings['get'] = $sw.ElapsedMilliseconds
        & $addErrors 'GET' @(Test-GetResponse -Response $getResult -Id $id -ExpectedTitle $title -ExpectedBodyContains "iteration $i")
        ...
        $sw.Stop()
        $timings['search'] = $sw.ElapsedMilliseconds
        & $addErrors 'SEARCH' @(Test-SearchResponse -Response $searchResult -Id $id)
        ...
        if ($doSemantic) {
            ...
            $sw.Stop()
            $timings['semantic'] = $sw.ElapsedMilliseconds
            & $addErrors 'SEMANTIC' @(Test-SearchResponse -Response $semResult -MinMatches 1)
        }
        ...
        $updatedBody = "$body UPDATED"
        $sw = [System.Diagnostics.Stopwatch]::StartNew()
        $updateResult = & $invokeClient @{ Action = 'add'; Id = $id; Title = $updatedTitle; Body = $updatedBody; Overwrite = $true }
        $sw.Stop()
        $timings['update'] = $sw.ElapsedMilliseconds
        & $addErrors 'UPDATE' @(Test-AddResponse -Response $updateResult -Id $id -ExpectedMutation overwritten)

        # VERIFY UPDATE
        $sw = [System.Diagnostics.Stopwatch]::StartNew()
        $verifyUpdateResult = & $invokeClient @{ Action = 'get'; Id = $id }
        $sw.Stop()
        $timings['verify_update'] = $sw.ElapsedMilliseconds
        & $addErrors 'VERIFY_UPDATE' @(Test-GetResponse -Response $verifyUpdateResult -Id $id -ExpectedTitle $updatedTitle -ExpectedBodyContains 'UPDATED')
        ...
        if (-not $noDelete) {
            ...
            $sw.Stop()
            $timings['remove'] = $sw.ElapsedMilliseconds
            & $addErrors 'REMOVE' @(Test-RemoveResponse -Response $removeResult -Id $id)
            ...
            $sw.Stop()
            $timings['verify_deleted'] = $sw.ElapsedMilliseconds
            & $addErrors 'VERIFY_DELETED' @(Test-DeletedResponse -Response $verifyResult -Id $id)
        }
...
$opTimings = @{}
foreach ($op in @('add','get','search','semantic','update','verify_update','remove','verify_deleted')) {
...
if ($EmbeddingsCompute) {
    ...
    if ($embErrors.Count -eq 0) {
        ...
        Write-Host ("  FAIL: {0}" -f ($embErrors -join '; ')) -ForegroundColor Red
        $postValidationErrors += "Embedding compute: $($embErrors -join '; ')"
    }
}
...
if ($EmbeddingsProjection) {
    ...
    if ($projErrors.Count -eq 0) {
        ...
        Write-Host ("  FAIL: {0}" -f ($projErrors -join '; ')) -ForegroundColor Red
        $postValidationErrors += "Embedding projection: $($projErrors -join '; ')"
    }
}

if ($postValidationErrors.Count -gt 0) {
    Write-Host ""
    Write-Host "=== Post-run validation errors ===" -ForegroundColor Red
    foreach ($e in $postValidationErrors) { Write-Host "  $e" -ForegroundColor Red }
    exit 1
}
