...
$ValidationScript = Join-Path $PSScriptRoot 'crud-response-validation.ps1'
if (-not (Test-Path $ValidationScript)) {
    Write-Error "Validation helper not found: $ValidationScript"
    return
}
. $ValidationScript
...
function Record-Op {
    ...
    $stats.TotalOps++
    $errors = if ($ValidationErrors.Count -gt 0) {
        @($ValidationErrors)
    } else {
        @(Test-ClientResponse -Response $Result -Operation $Phase)
    }
    if ($errors.Count -eq 0) {
        $stats.Succeeded++
    } else {
        $stats.Failed++
        [void]$stats.Errors.Add("[$Phase] $Id : $($errors -join '; ')")
    }
}
...
$baselineErrors = @(Test-ListResponse -Response $baseline)
if ($baselineErrors.Count -gt 0) {
    Write-Error "Baseline list validation failed: $($baselineErrors -join '; ')"
    return
}
$baselineCount = 0
...
for ($cycle = 1; $cycle -le $Cycles; $cycle++) {
    ...
    Measure-Phase "CREATE ($Count)" {
        $createResults = $ids | ForEach-Object -ThrottleLimit $Parallel -Parallel {
            ...
            try {
                ...
                $json = ConvertFrom-ClientOutput $raw
                $errors = @(Test-AddResponse -Response $json -Id $id -ExpectedMutation created)
                [PSCustomObject]@{ Id = $id; Success = ($errors.Count -eq 0); Error = ($errors -join '; ') }
            } catch {
    ...
    Measure-Phase "READ ($($sampleIds.Count) samples)" {
        foreach ($id in $sampleIds) {
            ...
            Record-Op 'READ' $result $id @(Test-GetResponse -Response $result -Id $id -ExpectedTitle "Stress Test: $id" -ExpectedBodyContains $id)
        }
    } | Out-Null
    ...
    Measure-Phase "LIST" {
        $expectId = $ids[0]
        $result = Invoke-Client @{ Action = 'list'; Limit = 200; ExpectId = $expectId }
        Record-Op 'LIST' $result 'all' @(Test-ListResponse -Response $result -Id $expectId)
    } | Out-Null

    # ── SEARCH ───────────────────────────────────────────────────────────
    Measure-Phase "SEARCH (keyword)" {
        $result = Invoke-Client @{ Action = 'search'; Keywords = @($prefix); Limit = 100 }
        Record-Op 'SEARCH' $result 'keyword' @(Test-SearchResponse -Response $result -MinMatches ([Math]::Min($Count, 100)))
    } | Out-Null
    ...
    if ($SemanticSearch) {
        Measure-Phase "SEMANTIC SEARCH" {
            ...
            Record-Op 'SEMANTIC' $result 'semantic-1' @(Test-SearchResponse -Response $result -MinMatches 1)
            $result2 = Invoke-Client @{ Action = 'search'; Keywords = @("instruction $prefix"); Mode = 'semantic' }
            Record-Op 'SEMANTIC' $result2 'semantic-2' @(Test-SearchResponse -Response $result2 -MinMatches 1)
        } | Out-Null
    }
    ...
    if ($EmbeddingsCompute) {
        Measure-Phase "EMBEDDINGS COMPUTE" {
            ...
            if ($embErrors.Count -eq 0) {
                ...
                [void]$stats.Errors.Add("[EMBEDDINGS] compute : $($embErrors -join '; ')")
            }
        } | Out-Null
        ...
        Measure-Phase "EMBEDDINGS PROJECTION" {
            ...
            if ($projErrors.Count -eq 0) {
                ...
                $stats.TotalOps++; $stats.Failed++
                [void]$stats.Errors.Add("[EMBEDDINGS] projection : $($projErrors -join '; ')")
            }
        } | Out-Null
    }
    ...
    Measure-Phase "UPDATE ($($updateIds.Count) overwrites)" {
        foreach ($id in $updateIds) {
            ...
            Record-Op 'UPDATE' $result $id @(Test-AddResponse -Response $result -Id $id -ExpectedMutation overwritten)
            $verify = Invoke-Client @{ Action = 'get'; Id = $id }
            Record-Op 'VERIFY_UPDATE' $verify $id @(Test-GetResponse -Response $verify -Id $id -ExpectedTitle "UPDATED: $id" -ExpectedBodyContains 'overwritten during stress testing')
        }
    } | Out-Null
    ...
    Measure-Phase "TRACK ($($trackIds.Count) signals)" {
        foreach ($id in $trackIds) {
            ...
            Record-Op 'TRACK' $result $id @(Test-UsageResponse -Response $result -Id $id)
        }
    } | Out-Null
    ...
    Measure-Phase "HOTSET" {
        ...
        Record-Op 'HOTSET' $result 'top10' @(Test-HotsetResponse -Response $result)
    } | Out-Null
    ...
    Measure-Phase "DELETE ($Count)" {
        foreach ($id in $ids) {
            ...
            Record-Op 'DELETE' $result $id @(Test-RemoveResponse -Response $result -Id $id)
        }
    } | Out-Null

    # ── VERIFY DELETE (sampled) ──────────────────────────────────────────
    $verifyDeleteIds = $ids | Get-Random -Count ([Math]::Min(10, $Count))
    Measure-Phase "VERIFY DELETE ($($verifyDeleteIds.Count) samples)" {
        foreach ($id in $verifyDeleteIds) {
            $result = Invoke-Client @{ Action = 'get'; Id = $id }
            Record-Op 'VERIFY_DELETE' $result $id @(Test-DeletedResponse -Response $result -Id $id)
        }
    } | Out-Null
...
if ($EmbeddingsCompute) {
    ...
    if ($embErrors.Count -eq 0) {
        ...
        $stats.TotalOps++; $stats.Failed++
        [void]$stats.Errors.Add("[EMBEDDINGS] post-compute : $($embErrors -join '; ')")
    }
    ...
    if ($projErrors.Count -eq 0) {
        ...
        Write-Host ("  Projection FAIL: {0}" -f ($projErrors -join '; ')) -ForegroundColor Red
        $stats.TotalOps++; $stats.Failed++
        [void]$stats.Errors.Add("[EMBEDDINGS] post-projection : $($projErrors -join '; ')")
    }
    Write-Host ""
}
...
if ($postHealthErrors.Count -gt 0) {
    $stats.TotalOps++; $stats.Failed++
    [void]$stats.Errors.Add("[HEALTH] post-test : $($postHealthErrors -join '; ')")
}
$postList = Invoke-Client @{ Action = 'list'; Limit = 1 }
$postListErrors = @(Test-ListResponse -Response $postList)
if ($postListErrors.Count -gt 0) {
    $stats.TotalOps++; $stats.Failed++
    [void]$stats.Errors.Add("[LIST] post-test : $($postListErrors -join '; ')")
}
$postCount = 0
