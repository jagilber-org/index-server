...
> Index Server — Enterprise instruction indexing platform for AI assistant governance. Aligned to the layered hook security template v1.31.0.
...
| SH-4 | File-system operations influenced by user input MUST validate that resolved paths stay within an intended base directory; unchecked path traversal is prohibited. [Review-only: no mechanical enforcement exists; see #585.] | error |
| SH-5 | Administrative servers, dashboards, and control-plane endpoints MUST default to loopback-only binding unless broader exposure is an explicit documented opt-in. [Review-only: no mechanical enforcement exists; see #585.] | error |
| SH-6 | Production code MUST keep TLS certificate verification enabled; self-signed or private-CA scenarios MUST use explicit trust configuration rather than disabling verification. [Review-only: no mechanical enforcement exists; see #585.] | error |
| SH-7 | API keys, admin tokens, and similar bearer credentials MUST be accepted through Authorization or other non-URL secret-bearing headers; query-parameter authentication is prohibited. [Review-only: no mechanical enforcement exists; see #585.] | error |
...
| SH-11 | Repositories with security scanning hooks SHOULD include adversarial test fixtures that verify scanner effectiveness against known bypass patterns, evasion techniques, and edge cases. | warning |
...
| ID | Rule | Severity |
|----|------|----------|
| AR-1 | Designs MUST default to the simplest structure that satisfies the current requirement set and documented enterprise constraints. [Review-only: no mechanical enforcement exists; see #585.] | error |
...
| A-3 | IndexContext is the single source of truth for instruction state (writeEntry + ensureLoaded). [Review-only: no mechanical enforcement exists; see #585.] | error |
| A-4 | Schema version must be tracked via src/versioning/schemaVersion.ts. | warning |
| A-5 | Audit logging (logAudit) is required for all mutation operations. [Review-only: no mechanical enforcement exists; see #585.] | error |
...
| A-7 | Canonical seeds in seedBootstrap.ts must contain only generalized, public-safe content about the index server, Index operations, and bootstrapping — no environment-specific, org-specific, or custom configuration data. [Review-only: no mechanical enforcement exists; see #585.] | error |
...
| OB-3 | Exceptions, failed operations, and unexpected states MUST be logged with enough stack or call-site context to support direct troubleshooting when the runtime can provide it. [Review-only: no mechanical enforcement exists; see #585.] | error |
...
| TS-6 | New features MUST include test coverage. [Review-only: no mechanical enforcement exists; see #585.] | error |
| TS-7 | Functional tests MUST validate full pipeline round-trips. [Review-only: no mechanical enforcement exists; see #585.] | error |
| TS-8 | TDD red/green is NON-NEGOTIABLE: tests MUST be written and verified FAILING before implementation code; failing test first, fix second (red-green-refactor). [Review-only: no mechanical enforcement exists; see #585.] | error |
| TS-9 | Every bug fix MUST start with a failing regression test: write a test that reproduces the failure, then fix and verify pass. [Review-only: no mechanical enforcement exists; see #585.] | error |
| TS-10 | Tests MUST exercise the REAL production code; toy reimplementations and stub copies of the code under test are prohibited. [Review-only: no mechanical enforcement exists; see #585.] | error |
| TS-11 | Playwright e2e tests MUST cover dashboard UI, instruction CRUD lifecycle, and search functionality. [Review-only: no mechanical enforcement exists; see #585.] | error |
| TS-12 | Bug-prone handlers and complex code paths MUST have >= 5 test cases covering normal, edge, error, boundary, and concurrent scenarios. [Review-only: no mechanical enforcement exists; see #585.] | error |
...
| PB-1 | All active repos MUST follow the dual-repo pattern: private dev repo for all development, public pub repo as read-only mirror. [Review-only: no mechanical enforcement exists; see #585.] | error |
...
| ID | Rule | Severity |
|----|------|----------|
| CQ-1 | Source files SHOULD target <=600 lines; MUST NOT exceed 1000 (template literals exempt). | error |
| CQ-2 | Each module MUST have a single primary responsibility; god-modules that mix unrelated concerns are prohibited. [Review-only: no mechanical enforcement exists; see #585.] | error |
| CQ-3 | Source code MUST follow the project declared layered architecture; cross-layer imports MUST flow downward only. [Review-only: no mechanical enforcement exists; see #585.] | error |
| CQ-4 | Circular dependencies are prohibited; module dependency graphs MUST be acyclic. [Review-only: no mechanical enforcement exists; see #585.] | error |
| CQ-5 | Dead code MUST be removed before commit; unused imports, variables, and unreachable branches are not allowed. | warning |
| CQ-6 | Error handling MUST be explicit; swallowed exceptions (empty catch blocks) are prohibited — errors MUST be logged, re-thrown, or handled with documented rationale. [Review-only: no mechanical enforcement exists; see #585.] | error |
...
| ID | Rule | Severity |
|----|------|----------|
| DI-1 | All stateful data MUST persist to disk; startup MUST auto-restore. [Review-only: no mechanical enforcement exists; see #585.] | error |
| DI-2 | Corrupted or empty files MUST result in safe empty state, not crashes. [Review-only: no mechanical enforcement exists; see #585.] | error |
| DI-3 | Schema changes to persisted files MUST include migration logic or be backwards-compatible. | warning |
| DI-4 | Write paths MUST mirror read paths for normalization, migration, and validation. Any record passing through a writer MUST run the same migration and validation sequence the loader runs at read time, so round-trips are symmetric. | error |
...
- **precommitWorkflow**: `.github/workflows/precommit.yml`
- **license**: `LICENSE`
- **constitutionSyncEngine**: `sync-constitution.cjs`

---
*Generated by sync-constitution.cjs from constitution.json v2.11.0*
