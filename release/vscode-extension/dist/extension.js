...
async function configureMcpClient(context) {
    ...
    if (format === 'vscode' || format === 'both') {
        actions.push('Open mcp.json');
    }
    if (format === 'copilot-cli' || format === 'both') {
        actions.push('Open mcp-config.json');
    }
    const writeAction = await vscode.window.showInformationMessage(`MCP configuration generated (${formatPicked.label}). Save it to the appropriate config file.`, ...actions);
 ...
 */
function buildProfileEnvBlock(profile, opts) {
 ...
 */
function generateJsoncConfig(mcpConfig, profile, format) {
