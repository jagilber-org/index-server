...
| `INDEX_SERVER_SQLITE_VEC_ENABLED` | `on (sqlite backend)` | Auto-enabled when `INDEX_SERVER_STORAGE_BACKEND=sqlite`; set `0` to opt out. Requires Node.js ≥ 22.13.0. Falls back to JSON if native binary unavailable. |
