...
| Body oversize | exceeds `bodyMaxLength` | truncated to limit | `bodyTruncated` + soft warning |
...
Body size enforcement is configurable via `INDEX_SERVER_BODY_MAX_LENGTH` (default: 20000, range: 1000–1000000).
The JSON schema ceiling is 1MB; the runtime config controls the effective limit. Bodies exceeding the configured
limit are always truncated (no hard reject). The near-limit soft warning fires at 90% of the configured limit.

| Limit | Enforcement | Soft Warning Counter |
|-------|-------------|---------------------|
| Body length > `bodyMaxLength` | Truncated to limit | `body:truncated` (plus salvage) |
| Body length 90–100% of limit | Accept | `body:near-limit` |
...
3. Keep `body` under 90% of configured limit ideally (default: <18k, absolute max configurable via `INDEX_SERVER_BODY_MAX_LENGTH`)
...
```text
[Index:skip] example.json: schema: /body: must NOT have more than 20000 characters
```
