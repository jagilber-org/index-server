...
| `services/storage/factory.ts` | `INDEX_SERVER_STORAGE_BACKEND`, `INDEX_SERVER_SQLITE_PATH`, `INDEX_SERVER_SQLITE_WAL`, `INDEX_SERVER_SQLITE_MIGRATE_ON_START`, `INDEX_SERVER_SQLITE_VEC_ENABLED`, `INDEX_SERVER_SQLITE_VEC_PATH` | `storage.backend`, `storage.sqlitePath`, `storage.sqliteWal`, `storage.sqliteMigrateOnStart`, `storage.sqliteVecEnabled`, `storage.sqliteVecPath` | string/boolean | ✅ DONE — Uses `getRuntimeConfig().storage.*` via `featureConfig.parseStorageConfig()`. `sqliteVecEnabled` defaults to `false` (opt-in). Requires Node.js ≥ 22.13.0 for sqlite-vec. |
