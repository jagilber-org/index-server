...
> **Status**: Phase 1 complete (2026-04-10). All runtime `process.env` reads in application code have been consolidated behind `getRuntimeConfig()`. Runtime configuration uses `INDEX_SERVER_*` prefixed environment variables.
...
4. **Single env surface** – runtime reads only `INDEX_SERVER_*` prefixed environment variables.
...
| `server/index-server.ts` | `INDEX_SERVER_DISABLE_EARLY_STDIN_BUFFER`, `INDEX_SERVER_DISABLE_PPID_WATCHDOG`, `INDEX_SERVER_FATAL_EXIT_DELAY_MS`, `INDEX_SERVER_DASHBOARD`, `INDEX_SERVER_DASHBOARD_PORT`, `INDEX_SERVER_DASHBOARD_HOST`, `INDEX_SERVER_DASHBOARD_TRIES`, `INDEX_SERVER_MUTATION`, `INDEX_SERVER_BOOTSTRAP_AUTOCONFIRM`, `INDEX_SERVER_ENABLE_INDEX_SERVER_POLLER`, `INDEX_SERVER_POLL_PROACTIVE`, `INDEX_SERVER_IDLE_KEEPALIVE_MS`, `INDEX_SERVER_LOG_FILE`, `INDEX_SERVER_SHARED_SERVER_SENTINEL`, `MULTICLIENT_TRACE` | `server.disableEarlyStdinBuffer`, `server.disablePpidWatchdog`, `server.fatalExitDelayMs`, `dashboard.http.enable`, `dashboard.http.port`, `dashboard.http.host`, `dashboard.http.maxPortTries`, `mutation.enabled`, `server.bootstrap.autoconfirm`, `server.IndexPolling.enabled`, `server.IndexPolling.proactive`, `server.idleKeepaliveMs`, `logging.fileTarget`, `server.sharedSentinel`, `server.multicoreTrace` | boolean/number/string | Mutation gating is controlled by `mutation.enabled`. |
...
| `services/handlers.search.ts` | `INDEX_SERVER_SEARCH_OMIT_ZERO_QUERY` | `Index.omitZeroResultQuery` | boolean / false | Used by generated MCP config validation flows to avoid echoing deleted identifiers in zero-result search metadata. |
...
| `services/storage/factory.ts` | `INDEX_SERVER_STORAGE_BACKEND`, `INDEX_SERVER_SQLITE_PATH`, `INDEX_SERVER_SQLITE_WAL`, `INDEX_SERVER_SQLITE_MIGRATE_ON_START`, `INDEX_SERVER_SQLITE_VEC_ENABLED`, `INDEX_SERVER_SQLITE_VEC_PATH` | `storage.backend`, `storage.sqlitePath`, `storage.sqliteWal`, `storage.sqliteMigrateOnStart`, `storage.sqliteVecEnabled`, `storage.sqliteVecPath` | string/boolean | ✅ DONE — Uses `getRuntimeConfig().storage.*` via `featureConfig.parseStorageConfig()`. `sqliteVecEnabled` **auto-enables when `backend=sqlite`**; set `INDEX_SERVER_SQLITE_VEC_ENABLED=0` to opt out. `factory.ts` falls back to JSON automatically if the native extension fails. Requires Node.js ≥ 22.13.0 for sqlite-vec. |
