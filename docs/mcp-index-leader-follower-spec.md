...
POST /leader/tools/call
  Body: { "name": "index_search", "arguments": {...} }
...
| E2.2 | Follower index_dispatch list | Call `index_dispatch { action: "list" }` via follower | Returns same Index as leader |
| E2.3 | Follower index_dispatch get | Get specific instruction by ID via follower | Returns correct instruction with full body |
| E2.4 | Follower index_dispatch query | Full-text query via follower | Results match leader |
| E2.5 | Follower index_dispatch categories | Get categories via follower | Matches leader |
...
| E2.9 | Follower index_search (keyword) | Search by keywords via follower | Matches leader results |
| E2.10 | Follower index_dispatch diff | Diff with client hash via follower | Correct diff returned |
...
| E7.10 | INDEX_SERVER_DIR changes at runtime | Change env var and call index_reload via follower | Leader reloads from new directory, all followers see updated Index |
