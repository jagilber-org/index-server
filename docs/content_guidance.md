...
Instruction body length is enforced at runtime via `INDEX_SERVER_BODY_MAX_LENGTH` (default: 20000 chars, range: 1000–1000000).

- Bodies exceeding the configured limit are **truncated** (not rejected) with a `bodyTruncated` salvage counter.
- A `body:near-limit` soft warning fires when body length exceeds 90% of the configured limit.
- The JSON schema ceiling is 1MB; the runtime config controls the effective enforcement limit.
- Keep bodies under 90% of the configured limit when possible to avoid truncation artifacts.
- For very large reference data, consider splitting across multiple instruction files or linking to external resources.
