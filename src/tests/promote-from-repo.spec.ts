...
describe('promote_from_repo', () => {
  ...
  describe('promotion-map.json', () => {
    ...
    it('returns a specific failure for invalid source scope metadata instead of throwing', async () => {
      const repoDir = path.join(TMP_BASE, 'invalid-scope-repo');
      ensureDir(repoDir);
      fs.writeFileSync(path.join(repoDir, 'README.md'), '# Test Repo');

      writeJson(path.join(repoDir, '.specify', 'config', 'promotion-map.json'), {
        sources: [
          {
            file: 'README.md',
            id: 'test-promote-invalid-scope',
            title: 'Invalid Scope',
            scope: ['docs'],
          },
        ],
      ...
      const result = await callPromote({ repoPath: repoDir, dryRun: true, scope: 'docs' });
      expect(result.failed).toEqual([
        {
          id: 'test-promote-invalid-scope',
          error: 'Invalid source category/scope: expected string',
        },
      ]);
      expect(result.promoted).toEqual([]);
      expect(result.dryRunEntries).toEqual([]);
    });

    it('returns a specific failure for invalid source path metadata instead of throwing', async () => {
      const repoDir = path.join(TMP_BASE, 'invalid-path-repo');
      ensureDir(repoDir);
        ...
        sources: [
          {
            ...
            instructionId: 'test-promote-invalid-path',
            title: 'Invalid Path',
            category: 'docs',
          },
        ],
      ...
      expect(result.failed).toEqual([
        {
          id: 'test-promote-invalid-path',
          error: 'Invalid source path/file: expected string',
        },
      ]);
    });

    it('returns a specific failure for invalid source id metadata instead of throwing', async () => {
      const repoDir = path.join(TMP_BASE, 'invalid-id-repo');
      ensureDir(repoDir);
      fs.writeFileSync(path.join(repoDir, 'README.md'), '# Test Repo');
      ...
      expect(result.failed).toEqual([
        {
          id: '*',
          error: 'Invalid source id/instructionId: expected string',
        },
      ]);
    });

    it('reports malformed top-level promotion-map.json instead of silently ignoring it', async () => {
      const repoDir = path.join(TMP_BASE, 'invalid-map-repo');
      ensureDir(repoDir);
      writeJson(path.join(repoDir, 'instructions', 'fallback-promoted.json'), makeMinimalInstruction('fallback-promoted'));
      ensureDir(path.join(repoDir, '.specify', 'config'));
      fs.writeFileSync(path.join(repoDir, '.specify', 'config', 'promotion-map.json'), '{invalid json');
      ...
      expect(failed[0]).toMatchObject({ id: 'promotion-map.json' });
      expect(failed[0].error).toContain('Invalid promotion map:');
      expect(result.dryRunEntries).toEqual([
        {
          id: 'fallback-promoted',
          title: 'Test instruction: fallback-promoted',
          action: 'add',
        },
      ]);
    });
