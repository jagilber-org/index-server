...
// Verifies every accepted on-disk instruction with a sourceHash field matches
// the same normalized body shape used by ClassificationService and
// integrity_verify. This guards against accidental manual edits to body without
// hash refresh and catches loader/raw-body normalization drift.
...
const INSTRUCTION_DIRS = [
  process.env.INDEX_SERVER_DIR || path.join(process.cwd(), 'instructions'),
  ...
  for (const dir of INSTRUCTION_DIRS) {
    it(`all sourceHash values match body in ${path.basename(dir)}`, async () => {
      ...
      for (const f of files) {
        ...
        const recomputed = sha256(data.body.trim());
