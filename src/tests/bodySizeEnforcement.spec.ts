 ...
 * 2. Exact boundary enforcement (bodyMaxLength + 1 rejected, bodyMaxLength accepted)
...
describe('body size enforcement', () => {
  ...
  it('rejects body exceeding bodyMaxLength with body_too_large error', async () => {
    const oversizedBody = 'x'.repeat(bodyMaxLength + 1);
  ...
  it('rejects body at exactly bodyMaxLength + 1', async () => {
    const justOverBody = 'a'.repeat(bodyMaxLength + 1);
    const resp = await add({
  ...
  it('accepts body at exactly bodyMaxLength', async () => {
    const exactBody = 'b'.repeat(bodyMaxLength);
    const resp = await add({
  ...
  it('accepts body under bodyMaxLength', async () => {
  ...
  it('rejects oversized body via dispatch add action', async () => {
    const oversizedBody = 'q'.repeat(bodyMaxLength + 200);
  ...
  it('rejects oversized entries in import without silent truncation', async () => {
    const oversizedBody = 'w'.repeat(bodyMaxLength + 300);
  ...
  it('import does not silently truncate oversized bodies', async () => {
    const oversizedBody = 'v'.repeat(bodyMaxLength + 100);
    const resp = await importHandler({
      entries: [
  ...
  it('get returns full body without truncation', async () => {
    ...
    const body = 'd'.repeat(bodyMaxLength - 100);
    await add({
