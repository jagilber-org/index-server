...
const instructionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'index-server-test-metahints-'));
...
// Test always seeds its own instruction data — no skip needed
// (previously skipped when repo root instructions/ was empty)
...
describe('_meta hints in search responses', () => {
  beforeEach(() => {
    seedInstruction();
  });
...
describe('_meta hints in dispatch responses', () => {
  beforeEach(() => {
    seedInstruction();
  });
