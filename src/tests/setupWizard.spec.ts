...
describe('Setup Wizard Multi-Target Config', () => {
  ...
  it('should write VS Code global config to user mcp.json', () => {
    ...
    try {
      ...
      // vscode-global is a user-machine-local config: the node binary is pinned
      // to an absolute path (process.execPath) so it never depends on the
      // launching client's inherited PATH. See fix/mcp-absolute-node-command.
      const globalCmd = config.servers?.['index-server']?.command ?? '';
      expect(path.isAbsolute(globalCmd), `command must be absolute: ${globalCmd}`).toBe(true);
      expect(/node(\.exe)?$/i.test(globalCmd), `command must point at node: ${globalCmd}`).toBe(true);
  ...
  it('should merge existing Copilot CLI mcp-config.json and create a backup', () => {
    ...
    try {
      ...
      // index-server is (re)generated by buildServerEntry for copilot-cli, which
      // pins the node binary to an absolute path; the pre-existing entry is
      // preserved verbatim. See fix/mcp-absolute-node-command.
      const cliCmd = merged.mcpServers?.['index-server']?.command ?? '';
      expect(path.isAbsolute(cliCmd), `command must be absolute: ${cliCmd}`).toBe(true);
      expect(/node(\.exe)?$/i.test(cliCmd), `command must point at node: ${cliCmd}`).toBe(true);
