...
describe('Instruction CRUD Full Lifecycle', () => {
  ...
  afterAll(() => {
    // Cleanup all created test instructions
  ...
  it('should enforce body size limits on add', async () => {
    const id = uniqueId();
    createdIds.push(id);
    const oversizedBody = 'x'.repeat(25000); // exceeds default 20000 limit
    ...
    try {
      ...
      // Should either throw or return error in response
      const r = result as Record<string, unknown>;
      expect(r.error || r.isError).toBeTruthy();
    } catch (e) {
      // Expected — body too large
      expect((e as Error).message.toLowerCase()).toContain('body');
    }
  });
