...
function parseIndexConfig(): IndexConfig {
  ...
  if (process.env.INDEX_SERVER_BODY_MAX_LENGTH !== undefined) {
    try { process.stderr.write(JSON.stringify({ ts: new Date().toISOString(), level: 'WARN', msg: '[runtimeConfig] INDEX_SERVER_BODY_MAX_LENGTH is no longer recognized. Use INDEX_SERVER_BODY_WARN_LENGTH instead (default: 100000).', pid: process.pid }) + '\n'); } catch { /* ignore */ }
  }
  return {
