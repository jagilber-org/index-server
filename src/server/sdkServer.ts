    ...
    try {
      ...
      setImmediate(() => {
        try {
          ...
          initFrameLog('ready_emit_scheduled', { reason: 'initialize-handler' });
          emitReadyGlobal(server,'initialize-handler');
        } catch { /* ignore */ }
      });
      return result;
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      try { process.stderr.write(`[startup] initialize_handler_failed ${message}\n`); } catch { /* ignore */ }
      throw {
        code: -32603,
