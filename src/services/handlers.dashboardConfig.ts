...
import { readOverlay, shadowedEnv, registerReadonlyFlags } from '../config/runtimeOverrides';
...
/**
 * Reload-behavior tier for a flag (plan §2.1, issue #359).
 *
 *   - dynamic           re-read on every consumer call site; safe to change at runtime
 *   - next-request      picked up by the next operation; not retroactive
 *   - restart-required  captured at boot/init; consumers cache; needs process restart
 *
 * Default classification when audit evidence is inconclusive: `restart-required`
 * (safe choice — never lies about effect). Promotion to `dynamic` requires
 * explicit `dynamicReadSite:true` evidence (T1 schema spec enforces this).
 *
 * NOTE for future maintainers: do not invent a fourth tier for WS-reconnect or
 * TLS-reload semantics. Both currently map to `restart-required` — see
 * `src/config/runtimeConfig.ts` header comment for the rationale.
 */
export type ReloadBehavior = 'dynamic' | 'next-request' | 'restart-required';

/**
 * Discriminated union for flag editability (Morpheus §2.1 revision #1).
 * `readonlyReason` is required for any non-editable flag; `readonlyDetail`
 * is an optional human-readable expansion surfaced as the row tooltip.
 */
export type FlagEditability =
  | { editable: true }
  | {
      editable: false;
      readonlyReason: 'derived' | 'deprecated' | 'reserved' | 'sensitive' | 'legacy';
      readonlyDetail?: string;
    };

/**
 * Validation/coercion constraints (Morpheus §2.1 revision #3).
 * `format` adds high-level semantic checks beyond min/max/pattern/enum.
 * v1 implements `url` and `port`; `path` falls back to `pattern`,
 * `duration-ms` to `min:0 + unit:'ms'`. No byte-size suffix parsing.
 */
...
export type FlagMeta = {
   ...
   * constructor or load/init/setup/build function AND classified as
   * anything other than `restart-required`.
   */
  dynamicReadSite?: boolean;
} & FlagEditability;

export type FlagRuntime = FlagMeta & {
  value?: string;
  enabled?: boolean;
  parsed?: unknown;
  docAnchor?: string;
  /**
   * True when an overlay entry exists for this flag AND a different `process.env`
   * value was present at boot (silent shadow). Surfaced by the dashboard so
   * operators can recognize when overlay state diverges from inherited ENV.
   * Plan §3.1 Morpheus revision #4.
   */
  overlayShadowsEnv: boolean;
  /**
   * For sensitive readonly flags (`editable:false` + `readonlyReason:'sensitive'`),
   * `value`/`parsed`/`enabled` are omitted from the snapshot to prevent secret
   * leakage into HTTP logs, browser DevTools, dashboard render state, and
   * session-history backups (#359 C2). `present` exposes the boolean
   * "configured / not configured" signal the UI needs.
   *
   * For non-sensitive flags this field is undefined.
   */
  present?: boolean;
};
...
export const FLAG_REGISTRY: FlagMeta[] = [
  ...
  { name:'INDEX_SERVER_DIR', category:'core', description:'Instructions catalog directory. Defaults to ./instructions relative to CWD.', stability:'stable', default:'./instructions', type:'string', since:'1.0.0', reloadBehavior:'restart-required', editable:true },
  ...
  { name:'INDEX_SERVER_MANIFEST_FASTLOAD', category:'manifest', description:'Preview fastload path (currently reserved).', stability:'reserved', default:'off', type:'boolean', since:'1.1.0', reloadBehavior:'restart-required', editable:false, readonlyReason:'reserved', readonlyDetail:'Reserved for future use' },
  { name:'INDEX_SERVER_ENABLE_INDEX_SERVER_POLLER', category:'manifest', description:'Enable background version marker poller (cross-process propagation).', stability:'diagnostic', default:'off', type:'boolean', since:'1.1.1', reloadBehavior:'restart-required', editable:true },
  ...
  { name:'INDEX_SERVER_TEST_STRICT_VISIBILITY', category:'instructions', description:'Test-only strict fallback path for immediate get/query discoverability.', stability:'diagnostic', default:'off', type:'boolean', since:'1.1.1', reloadBehavior:'restart-required', editable:true },
  ...
  { name:'INDEX_SERVER_TRACE_ALL', category:'tracing', description:'Force maximum trace verbosity.', stability:'diagnostic', default:'off', type:'boolean', since:'1.1.2', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_TRACE_PERSIST', category:'tracing', description:'Enable persistent JSONL trace output (auto file).', stability:'stable', default:'off', type:'boolean', since:'1.1.2', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_TRACE_FILE', category:'tracing', description:'Explicit trace output file path.', stability:'stable', default:'(unset)', type:'string', since:'1.1.2', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_TRACE_DIR', category:'tracing', description:'Directory for auto trace files.', stability:'stable', default:'./logs/trace', type:'string', since:'1.1.2', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_TRACE_MAX_FILE_SIZE', category:'tracing', description:'Rotate trace file after exceeding N bytes (0=off).', stability:'stable', default:'0', type:'number', since:'1.1.2', reloadBehavior:'restart-required', editable:true, validation:{ min:0, unit:'bytes' } },
  { name:'INDEX_SERVER_TRACE_CATEGORIES', category:'tracing', description:'Comma/space list of allowed trace categories (filter).', stability:'stable', default:'(all)', type:'string', since:'1.1.2', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_TRACE_SESSION', category:'tracing', description:'Explicit trace session id.', stability:'stable', default:'(random)', type:'string', since:'1.1.2', reloadBehavior:'restart-required', editable:true },
  ...
  // Usage & metrics
  { name:'INDEX_SERVER_RATE_LIMIT', category:'usage', description:'Dashboard HTTP API and usage-tracking rate limit, in requests per minute. 0 (default) disables rate limiting. Bulk import/export/backup/restore routes are unconditionally exempt.', stability:'stable', default:'0', type:'number', since:'1.27.0', reloadBehavior:'restart-required', editable:true, validation:{ min:0, unit:'req/min' } },
  { name:'INDEX_SERVER_DISABLE_USAGE_CLAMP', category:'usage', description:'Disable initial usage count clamp logic.', stability:'diagnostic', default:'off', type:'boolean', since:'1.1.1', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_USAGE_FLUSH_MS', category:'usage', description:'Override usage flush debounce interval.', stability:'diagnostic', default:'75', type:'number', since:'1.1.1', reloadBehavior:'restart-required', editable:true, validation:{ min:0, unit:'ms' } },

  // Validation / schema
  { name:'INDEX_SERVER_VALIDATION_MODE', category:'validation', description:'Schema validation engine selection (ajv|zod|auto).', stability:'stable', default:'zod', type:'string', since:'1.1.2', reloadBehavior:'restart-required', editable:true, validation:{ enum:['ajv','zod','auto'] } },

  // Handshake / transport / performance diagnostics
  { name:'INDEX_SERVER_DISABLE_EARLY_STDIN_BUFFER', category:'diagnostics', description:'Disable early stdin buffering (compare fragmentation behavior).', stability:'diagnostic', default:'off', type:'boolean', since:'1.1.1', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_FATAL_EXIT_DELAY_MS', category:'diagnostics', description:'Delay before forced fatal exit (ms).', stability:'diagnostic', default:'15', type:'number', since:'1.1.1', reloadBehavior:'restart-required', editable:true, validation:{ min:0, unit:'ms' } },
  { name:'INDEX_SERVER_IDLE_KEEPALIVE_MS', category:'diagnostics', description:'Keepalive interval for idle transports.', stability:'stable', default:'30000', type:'number', since:'1.0.0', reloadBehavior:'restart-required', editable:true, validation:{ min:1000, unit:'ms' } },
  { name:'INDEX_SERVER_DISABLE_PPID_WATCHDOG', category:'diagnostics', description:'Disable parent-process watchdog. Required for dev sandbox launchers that spawn through a transient shell (e.g. cmd /c start /B node).', stability:'experimental', default:'off', type:'boolean', since:'1.28.14', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_ADD_TIMING', category:'diagnostics', description:'Embed per-tool timing phase marks in response envelope.', stability:'diagnostic', default:'off', type:'boolean', since:'1.1.1', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_TRACE_DISPATCH_DIAG', category:'diagnostics', description:'Extra dispatcher timing/phase logs (use INDEX_SERVER_TRACE=dispatchDiag).', stability:'diagnostic', default:'off', type:'boolean', since:'1.1.1', reloadBehavior:'restart-required', editable:true },

  // Stress / adversarial
  { name:'INDEX_SERVER_STRESS_DIAG', category:'stress', description:'Enable stress suite & escalated diagnostic loops.', stability:'diagnostic', default:'off', type:'boolean', since:'1.1.0', reloadBehavior:'restart-required', editable:true },

  // Auth / security (placeholders for future expansion)
  { name:'INDEX_SERVER_REQUIRE_AUTH_ALL', category:'auth', description:'Require auth for all tool calls (future integration).', stability:'experimental', default:'off', type:'boolean', since:'1.1.2', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_AUTH_KEY', category:'auth', description:'Static auth key / token (development only).', stability:'experimental', default:'(unset)', type:'string', since:'1.1.2', reloadBehavior:'restart-required', editable:false, readonlyReason:'sensitive', readonlyDetail:'Secret — set via env or secret manager only' },
  ...
  { name:'INDEX_SERVER_MODE', category:'multi-instance', description:'Instance mode: standalone (default), leader, follower, auto.', stability:'experimental', default:'standalone', type:'string', since:'1.8.5', reloadBehavior:'restart-required', editable:true, validation:{ enum:['standalone','leader','follower','auto'] } },
  { name:'INDEX_SERVER_LEADER_PORT', category:'multi-instance', description:'HTTP port for leader MCP transport (thin clients connect here).', stability:'experimental', default:'9090', type:'number', since:'1.8.5', reloadBehavior:'restart-required', editable:true, validation:{ min:1, max:65535, format:'port' } },
  { name:'INDEX_SERVER_HEARTBEAT_MS', category:'multi-instance', description:'Leader heartbeat interval (ms).', stability:'experimental', default:'5000', type:'number', since:'1.8.5', reloadBehavior:'restart-required', editable:true, validation:{ min:100, unit:'ms' } },
  { name:'INDEX_SERVER_STALE_THRESHOLD_MS', category:'multi-instance', description:'Stale leader threshold (ms) before follower promotes.', stability:'experimental', default:'15000', type:'number', since:'1.8.5', reloadBehavior:'restart-required', editable:true, validation:{ min:100, unit:'ms' } },

  // Semantic / embeddings
  { name:'INDEX_SERVER_SEMANTIC_ENABLED', category:'semantic', description:'Enable semantic search & embedding compute.', stability:'experimental', default:'off', type:'boolean', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_SEMANTIC_MODEL', category:'semantic', description:'HuggingFace embedding model id (e.g. Xenova/all-MiniLM-L6-v2).', stability:'experimental', default:'(default)', type:'string', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_SEMANTIC_DEVICE', category:'semantic', description:'Inference device: cpu | cuda | dml.', stability:'experimental', default:'cpu', type:'string', since:'1.20.0', reloadBehavior:'restart-required', editable:true, validation:{ enum:['cpu','cuda','dml'] } },
  { name:'INDEX_SERVER_SEMANTIC_CACHE_DIR', category:'semantic', description:'Directory for downloaded model artifacts.', stability:'experimental', default:'./data/models', type:'string', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_SEMANTIC_LOCAL_ONLY', category:'semantic', description:'Disallow remote model downloads (offline mode).', stability:'experimental', default:'on', type:'boolean', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_EMBEDDING_PATH', category:'semantic', description:'Path to embeddings JSON cache file.', stability:'experimental', default:'./data/embeddings.json', type:'string', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_AUTO_EMBED_ON_IMPORT', category:'semantic', description:'Auto-compute embeddings after zip import / restore (when semantic enabled).', stability:'experimental', default:'on', type:'boolean', since:'1.27.3', reloadBehavior:'restart-required', editable:true },

  // Storage backend
  { name:'INDEX_SERVER_STORAGE_BACKEND', category:'storage', description:'Instruction store backend: json (default) | sqlite (experimental).', stability:'experimental', default:'json', type:'string', since:'1.25.0', reloadBehavior:'restart-required', editable:true, validation:{ enum:['json','sqlite'] } },
  { name:'INDEX_SERVER_SQLITE_PATH', category:'storage', description:'SQLite database file path.', stability:'experimental', default:'./data/index.db', type:'string', since:'1.25.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_SQLITE_WAL', category:'storage', description:'Enable SQLite WAL journaling.', stability:'experimental', default:'on', type:'boolean', since:'1.25.0', reloadBehavior:'restart-required', editable:true },
  ...
  // Dashboard / TLS
  { name:'INDEX_SERVER_DASHBOARD_TLS', category:'dashboard', description:'Enable HTTPS for the admin dashboard.', stability:'stable', default:'off', type:'boolean', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_DASHBOARD_TLS_CERT', category:'dashboard', description:'TLS certificate path.', stability:'stable', default:'(unset)', type:'string', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_DASHBOARD_TLS_KEY', category:'dashboard', description:'TLS private key path.', stability:'stable', default:'(unset)', type:'string', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_DASHBOARD_TLS_CA', category:'dashboard', description:'Optional TLS CA bundle path.', stability:'stable', default:'(unset)', type:'string', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_DASHBOARD_GRAPH', category:'dashboard', description:'Enable dashboard graph rendering.', stability:'stable', default:'off', type:'boolean', since:'1.18.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_HTTP_METRICS', category:'dashboard', description:'Expose Prometheus-style HTTP metrics.', stability:'stable', default:'on', type:'boolean', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  ...
  { name:'INDEX_SERVER_LOG_PROTOCOL', category:'diagnostics', description:'Log MCP protocol frames.', stability:'diagnostic', default:'off', type:'boolean', since:'1.0.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_EVENT_BUFFER_SIZE', category:'diagnostics', description:'Capacity of in-memory WARN/ERROR ring buffer surfaced in dashboard Events panel.', stability:'stable', default:'500', type:'number', since:'1.27.3', reloadBehavior:'restart-required', editable:true, validation:{ min:1 } },
  { name:'INDEX_SERVER_EVENT_SILENT', category:'diagnostics', description:'Suppress redundant index-event logs.', stability:'diagnostic', default:'off', type:'boolean', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  ...
  { name:'INDEX_SERVER_MINIMAL_DEBUG', category:'diagnostics', description:'Minimal debug surface (reduces verbosity).', stability:'diagnostic', default:'off', type:'boolean', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_STRESS_MODE', category:'diagnostics', description:'Enable stress-test instrumentation.', stability:'diagnostic', default:'off', type:'boolean', since:'1.20.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_GRAPH_INCLUDE_PRIMARY_EDGES', category:'index', description:'Include primary edges in graph export.', stability:'experimental', default:'on', type:'boolean', since:'1.18.0', reloadBehavior:'restart-required', editable:true },
  { name:'INDEX_SERVER_GRAPH_LARGE_CATEGORY_CAP', category:'index', description:'Cap large-category fanout in graph export.', stability:'experimental', default:'(unset)', type:'number', since:'1.18.0', reloadBehavior:'restart-required', editable:true, validation:{ min:1 } },

  // Runtime overrides overlay (meta-flags — control the overlay layer itself; not themselves overlay-able).
  { name:'INDEX_SERVER_OVERRIDES_FILE', category:'core', description:'Path to the runtime overrides overlay JSON file (default data/runtime-overrides.json).', stability:'stable', default:'./data/runtime-overrides.json', type:'string', since:'1.29.0', reloadBehavior:'restart-required', editable:false, readonlyReason:'derived', readonlyDetail:'Pointing the dashboard at a different overlay file would be a chicken-and-egg — set via env or launcher.' },
  { name:'INDEX_SERVER_DISABLE_OVERRIDES', category:'core', description:'Kill-switch for the runtime overrides overlay (1 = ignore overlay file at boot).', stability:'stable', default:'off', type:'boolean', since:'1.29.0', reloadBehavior:'restart-required', editable:false, readonlyReason:'derived', readonlyDetail:'Cannot be set via overlay; controls whether the overlay is loaded.' },
];

// #359 H1 — register the readonly flag set with the runtime overrides layer so
// `writeOverride()` has defense-in-depth coverage even for future callers that
// bypass the admin route's validateFlagUpdate() gate. Side-effect at module
// load: by the time any consumer (admin route, CLI tool) imports this file,
// runtimeOverrides has the authoritative blocklist.
registerReadonlyFlags(FLAG_REGISTRY.filter(f => f.editable === false).map(f => f.name));

function parseValue(meta: FlagMeta): { value?:string; enabled?:boolean; parsed?:unknown; present?:boolean } {
  const raw = process.env[meta.name];
  // #359 C2 — sensitive readonly flags: never return plaintext. Emit only
...
export function getFlagRegistrySnapshot(): FlagRuntime[] {
  // Compute overlay/shadow context once per snapshot.
  let overlay: Record<string, string> = {};
  try { overlay = readOverlay(); } catch { overlay = {}; }
  const shadow = shadowedEnv();
  return FLAG_REGISTRY
    .slice()
    .sort((a, b) => a.category.localeCompare(b.category) || a.name.localeCompare(b.name))
    .map(m => {
      const inOverlay = Object.prototype.hasOwnProperty.call(overlay, m.name);
      const overlayShadowsEnv = inOverlay && shadow[m.name] !== undefined && shadow[m.name] !== overlay[m.name];
      return {
        ...m,
        ...parseValue(m),
        docAnchor: m.name.toLowerCase().replace(/_/g, '-'),
        overlayShadowsEnv,
      };
    });
}
