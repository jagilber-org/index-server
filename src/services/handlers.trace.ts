...
registerHandler('trace_dump', (p:{ file?: string }) => {
  ...
  try { const stat = fs.statSync(file); bytes = stat.size; const raw = JSON.parse(fs.readFileSync(file,'utf8')); if(raw && Array.isArray(raw.records)) size = raw.records.length; } catch { /* ignore */ } // lgtm[js/file-system-race] — file path containment to cwd validated above
