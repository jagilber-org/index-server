...
registerHandler('index_add', guard('index_add', (p: AddParams) => {
  ...
  const { bodyMaxLength } = getRuntimeConfig().index;
  ...
  const govKeys: (keyof ImportEntry)[] = ['version', 'owner', 'status', 'priorityTier', 'classification', 'lastReviewedAt', 'nextReviewDue', 'semanticSummary', 'contentType'];
