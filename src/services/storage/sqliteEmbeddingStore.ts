...
export class SqliteEmbeddingStore implements IEmbeddingStore {
  ...
  private initExtension(): void {
    ...
    const nmDir = require('path').resolve(__dirname, '..', '..', '..', 'node_modules');
    if (!resolved.startsWith(nmDir + require('path').sep) && !resolved.startsWith(nmDir + '/')) {
  ...
  load(): EmbeddingCacheData | null {
    ...
    if (rows.length === 0 && !hashRow) return null;
