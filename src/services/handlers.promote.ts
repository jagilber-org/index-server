...
interface PromotionMapLoadResult {
  map: PromotionMap | null;
  error?: string;
}
...
function loadPromotionMap(repoPath: string): PromotionMapLoadResult {
  const mapPath = path.join(repoPath, '.specify', 'config', 'promotion-map.json');
  if (!fs.existsSync(mapPath)) return { map: null };
  try {
    const raw = fs.readFileSync(mapPath, 'utf8');
    const parsed = JSON.parse(raw) as PromotionMap;
    if (!parsed.sources || !Array.isArray(parsed.sources)) {
      return { map: null, error: 'Invalid promotion map: expected object with sources array' };
    }
    return { map: parsed };
  } catch (error) {
    return { map: null, error: `Invalid promotion map: ${error instanceof Error ? error.message : String(error)}` };
  }
}
...
function scanMappedSources(
  ...
  for (const source of map.sources) {
    try {
      ...
      const rawSourcePath = source.path ?? source.file;
      const rawSourceId = source.instructionId ?? source.id;
      const rawSourceCategory = source.category || source.scope || (Array.isArray(source.categories) ? source.categories[0] : undefined) || 'instruction';

      if (rawSourcePath === undefined || rawSourceId === undefined) continue;
      if (typeof rawSourcePath !== 'string') {
        failed.push({ id: typeof rawSourceId === 'string' ? rawSourceId : '*', error: 'Invalid source path/file: expected string' });
        continue;
      }
      if (typeof rawSourceId !== 'string') {
        failed.push({ id: '*', error: 'Invalid source id/instructionId: expected string' });
        continue;
      }
      if (typeof rawSourceCategory !== 'string') {
        failed.push({ id: rawSourceId, error: 'Invalid source category/scope: expected string' });
        continue;
      }
      if (!matchesScope(rawSourceCategory, scope)) continue;

      const filePath = path.join(repoPath, rawSourcePath);
      ...
      failed.push({
        id: typeof (source.instructionId ?? source.id) === 'string' ? (source.instructionId ?? source.id) as string : '*',
        error: `Source scan failed: ${error instanceof Error ? error.message : String(error)}`,
      });
    }
  }
  return results;
...
registerHandler('promote_from_repo', (params: PromoteParams) => {
  ...
  // 1. Promotion map
  const mapLoad = loadPromotionMap(repoPath);
  if (mapLoad.error) {
    failed.push({ id: 'promotion-map.json', error: mapLoad.error });
  }
  if (mapLoad.map) {
    candidates.push(...scanMappedSources(repoPath, mapLoad.map, scope, failed));
  }
